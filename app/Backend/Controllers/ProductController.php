...
EOT
        ...
        return Admin::form(Product::class, function (Form $form) use ($cateArr, $majorCateArr, $postArr, $id) {
            ...
            $form->multipleImage('image_group','产品组图')->removable()->options([
                ...
                'deleteUrl' => url('backend/new_delete'),
                'uploadUrl' => url('backend/new_upload'),
                'deleteExtraData' => [
                    '_token'    => csrf_token(),
                    '_method'   => 'POST',
                ],
            ...
            $form->multipleFile('files','附件')->removable()->options([
                ...
                'autoOrientImage' => true,
                'deleteUrl' => url('backend/new_delete'),
                'uploadUrl' => url('backend/new_upload_file'),
                'deleteExtraData' => [
                    '_token'    => csrf_token(),
                    '_method'   => 'POST',
                ],
    ...
    public function uploadFile(Request $request)
    {
        if ($request->hasFile('files')) {
            ...
            $paths = [];
            foreach ($files as $file) {
                $paths[] = $file->storeAs(config('admin.upload.directory.file'), $file->getClientOriginalName());
            }
